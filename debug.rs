...
/// Bennani Kernel panic routine, without the infinite LED-blinking loop.
///
...
/// Bennani Kernel default panic routine.
///
/// **NOTE:** The supplied `writer` must be synchronous.
...
pub unsafe fn panic_banner<W: Write>(writer: &mut W, panic_info: &PanicInfo) {
    ...
    let _ = writer.write_fmt(format_args!(
        "\tKernel version {}\r\n",
        option_env!("Bennani Kernel_KERNEL_VERSION").unwrap_or("unknown")
    ));
}
...
fn write_header(writer: &mut DebugWriterWrapper, (file, line): &(&'static str, u32)) -> Result {
    ...
    writer.write_fmt(format_args!("Bennani Kernel_DEBUG({}): {}:{}: ", count, file, line))
}
